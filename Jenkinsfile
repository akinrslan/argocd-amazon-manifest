node {
    ...
    env.IMAGE = 'akinino/amazon'

    stage('Clone repository') {
             git branch: 'main', url: 'https://github.com/akinrslan/argocd-amazon-manifest.git'  
    }

    stage('Update GIT') {
            script {
                catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                    withCredentials([usernamePassword(credentialsId: 'akin-github', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
